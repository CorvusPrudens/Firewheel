...
/// This filter has the form: `y[n] = b0 x[n] + b1 x[n-1] - a1 y[n-1]`
/// `m` is the memory, storing `b1 x[n-1] - a1 y[n-1]` for use in the next iteration
#[derive(Default, Clone, Copy)]
...
impl Filter for FirstOrderFilter {
    ...
    fn process(&mut self, x: f32, coeffs: &Self::Coeffs) -> f32 {
        let y = self.m + coeffs.b0 * x;
        self.m = coeffs.b1 * x - coeffs.a1 * y;
        y
    }
...
// A biquad filter
/// This filter has the form: `y[n] = b0 x[n] + b1 x[n-1] + b2 x[n-2] - a1 y[n-1] - a2 y[n-2]`
/// `d1` stores `b1 x[n-1] - a1 y[n-1]` for use in the next iteration
/// `d2` stores `b2 x[n-1] - a2 y[n-1]` for use in the iteration after the next iteration
#[derive(Default, Clone, Copy)]
pub struct Biquad {
    d1: f32,
    d2: f32,
...
impl Filter for Biquad {
    ...
    fn is_silent(&self, eps: f32) -> bool {
        self.d1.abs() <= eps && self.d2.abs() <= eps
    }
}
